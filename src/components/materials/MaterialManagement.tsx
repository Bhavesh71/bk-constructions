...
export function MaterialManagement({ materials, isAdmin }: { materials: Material[]; isAdmin: boolean }) {
  ...
  async function confirmDeleteMaterial() {
    ...
    const result = await deleteMaterial(confirmDelete.id)
    if (result.success) { toast.success('Material removed'); router.refresh() }
    else toast.error(result.error || 'Failed to remove material')
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700 w-full max-w-sm p-6">
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                <h3 className="font-bold text-gray-900 dark:text-white text-sm">Delete Material?</h3>
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">
                  <span className="font-semibold text-gray-700 dark:text-slate-200">{confirmDelete.name}</span> will be permanently removed from the catalogue.
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={confirmDeleteMaterial}
                className="flex-1 px-4 py-2 rounded-xl text-sm font-semibold bg-red-500 text-white hover:bg-red-600 transition-colors">
                Delete
              </button>
            </div>
          </div>
