...
interface Material {
  ...
  totalSpent: number; totalQuantity: number; usageCount: number
}

const CATEGORIES = ['Binding', 'Steel', 'Aggregate', 'Masonry', 'Wood', 'Electrical', 'Plumbing', 'Finishing', 'Other']

export function MaterialManagement({ materials, isAdmin }: { materials: Material[]; isAdmin: boolean }) {
  const router = useRouter()
  const [showForm, setShowForm] = useState(false)
  const [editingId, setEditingId] = useState<string | null>(null)
  const [form, setForm] = useState({ name: '', unit: '', defaultRate: '', category: 'Binding' })
  const [loading, setLoading] = useState(false)
  const [filter, setFilter] = useState('All')
  const set = (k: string, v: string) => setForm(f => ({ ...f, [k]: v }))

  const categories = ['All', ...CATEGORIES.filter(c => materials.some(m => m.category === c))]
  ...
  function resetForm() { setShowForm(false); setEditingId(null); setForm({ name: '', unit: '', defaultRate: '', category: 'Binding' }) }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result.success) { toast.success(editingId ? 'Material updated' : 'Material added'); resetForm(); router.refresh() }
  ...
  async function handleDelete(id: string, name: string) {
    ...
    if (result.success) { toast.success('Material removed'); router.refresh() }
    else toast.error(result.error || 'Failed')
  }
          ...
          {showForm && (
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className="input" value={form.unit} onChange={e => set('unit', e.target.value)} placeholder="e.g. Bag, Kg, CFT, Nos" required />
              </div>
              <div>
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>Unit</th>
                <th>Default Rate</th>
                <th className="hidden md:table-cell">Usage Count</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="text-center text-gray-400 py-8">No materials in this category</td></tr>
              ) : (
                filtered.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td className="hidden md:table-cell text-gray-700">{m.usageCount}×</td>
                    <td className="hidden lg:table-cell text-financial font-semibold text-green-600">{formatCurrency(m.totalSpent)}</td>
                    {isAdmin && (
                      <td>
                        <div className="flex gap-1">
                          <button onClick={() => startEdit(m)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleDelete(m.id, m.name)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      </td>
                    )}
                  </tr>
